...
import { cancelNormalSelected, getCellValue, getInlineStringHTML, getStyleByCell, isInlineStringCell, moveToEnd, getFlowdata, handleFormulaInput, moveHighlightCell, escapeScriptTag, valueShowEs, createRangeHightlight, isShowHidenCR, israngeseleciton, escapeHTMLTag, isAllowEdit, getrangeseleciton, } from "@nde-sheet/core";
import { useContext, useEffect, useMemo, useRef, useCallback, useLayoutEffect, useState, } from "react";
...
const InputBox = () => {
    ...
    const lastKeyDownEventRef = useRef(null);
    const prevCellUpdate = usePrevious(context.luckysheetCellUpdate);
    const prevSheetId = usePrevious(context.currentSheetId);
    const [isHidenRC, setIsHidenRC] = useState(false);
    ...
    const selectActiveFormula = useCallback((e) => {
        const activeFormula = getActiveFormula();
    ...
    const onKeyDown = useCallback((e) => {
        ...
        if (e.key === "Escape" && context.luckysheetCellUpdate.length > 0) {
            ...
            if (document.getElementById("luckysheet-formula-search-c")) {
                ...
                if (!nextItem) {
                    nextItem =
                        formulaSearchContainer?.querySelector(".luckysheet-formula-search-item:first-child") || null;
                }
                clearSearchItemActiveClass();
                if (nextItem) {
    ...
    const onChange = useCallback((__, isBlur) => {
        // setInputHTML(html);
    ...
    const rowReadOnly = cfg.rowReadOnly || {};
    const colReadOnly = cfg.colReadOnly || {};
    const edit = !((colReadOnly[col_index] || rowReadOnly[row_index]) &&
        context.allowEdit === true);
    return (_jsxs("div", { className: "luckysheet-input-box", style: firstSelection && !context.rangeDialog?.show
            ? {
            ...
            : { left: -10000, top: -10000, display: "block" }, onMouseDown: (e) => e.stopPropagation(), onMouseUp: (e) => e.stopPropagation(), children: [_jsx("div", { className: "luckysheet-input-box-inner", style: firstSelection
                    ? {
                        minWidth: firstSelection?.width || 0,
                        minHeight: firstSelection?.height || 0,
                        ...inputBoxStyle,
                    }
                    : {}, children: _jsx(ContentEditable, { innerRef: (e) => {
                        // @ts-ignore
                        inputRef.current = e;
                    ...
                    }, onChange: onChange, onKeyDown: onKeyDown, onPaste: onPaste, allowEdit: edit ? !isHidenRC : edit }) }), document.activeElement === inputRef.current && (_jsxs(_Fragment, { children: [_jsx(FormulaSearch, { style: {
                            top: (firstSelection?.height_move || 0) + 4,
                        } }), _jsx(FormulaHint, { style: {
                            top: (firstSelection?.height_move || 0) + 4,
                        } })] }))] }));
};
