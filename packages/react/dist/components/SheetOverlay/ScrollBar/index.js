...
const ScrollBar = ({ axis }) => {
    ...
    return (_jsx("div", { ref: axis === "x" ? refs.scrollbarX : refs.scrollbarY, style: axis === "x"
            ? {
                left: context.rowHeaderWidth,
                width: `calc(100% - ${context.rowHeaderWidth}px)`,
            }
            : { height: "100%" }, className: `luckysheet-scrollbars luckysheet-scrollbar-ltr luckysheet-scrollbar-${axis}`, onScroll: () => {
            if (axis === "x") {
                setContext((draftCtx) => {
        ...
        }, children: _jsx("div", { style: axis === "x"
